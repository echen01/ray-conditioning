...
#SBATCH -J FFHQPre                           # Job name
#SBATCH -o FFHQPre.o                              # Name of stdout output file (%j expands to jobId)
#SBATCH -e FFHQPre.o                              # Name of stderr output file (%j expands to jobId)
...
#SBATCH --gres=gpu:3090:2                       # gpu:gpu_type:how_many_gpu
...
python /share/phoenix/nfs04/S7/emc348/stylegan3/train.py --outdir=/share/phoenix/nfs04/S7/emc348/stylegan3/training-runs --data=/share/cuvl/sh844/FFHQ_1024.zip --cfg=ray-cond --gpus=2 --batch=32 --batch-gpu=8 --gamma=1 --snap=20 --metrics=none --cond=1 --aug=noaug --resume=/share/phoenix/nfs04/S7/emc348/nerf/eg3d/stylegan2-ffhq-1024x1024.pkl
