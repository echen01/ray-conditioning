...
#SBATCH -J AFHQPre                           # Job name
#SBATCH -o AFHQPre.o                              # Name of stdout output file (%j expands to jobId)
#SBATCH -e AFHQPre.o                              # Name of stderr output file (%j expands to jobId)
...
python /share/phoenix/nfs04/S7/emc348/stylegan3/train.py --outdir=/share/phoenix/nfs04/S7/emc348/stylegan3/training-runs --data=/share/phoenix/nfs04/S7/emc348/nerf/eg3d/processed_afhq.zip --cfg=ray-cond --gpus=2 --batch=32 --gamma=5 --snap=20 --metrics=none --cond=1 --aug=noaug --resume=/share/phoenix/nfs04/S7/emc348/nerf/eg3d/afhqcat.pkl
