...
class SynthesisBlock(torch.nn.Module):
    def __init__(
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if self.ray_cond:
            intermediate_channels = out_channels + 3
        else:
    ...
    def forward(
        ...
        **layer_kwargs,
        ...
        if self.ray_cond:
            ...
            ray_embed = torch.cat([w_embed, rays1], dim=1)
            # self.depth1 = self.depth1.to(dtype=dtype, memory_format=memory_format)
            d = self.depth1(ray_embed)
            p = rays_od[:, :3, :, :] + d * rays_od[:, 3:, :, :]
        ...
        if self.is_last or self.architecture == "skip":
            # x_rays = torch.cat([x, rays_next], dim=1)
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(
        ...
        **block_kwargs,  # Arguments for SynthesisBlock.
        ...
        self.num_ws = 0
        self.depth = torch.nn.Sequential(
            Conv2dLayer(self.w_dim + 6, 16, 1, activation="lrelu"),
            Conv2dLayer(16, 3, 1, activation="lrelu"),
            Conv2dLayer(3, 1, 1, activation="relu"),
        )
        for res in self.block_resolutions:
            """if res >= 128:
                in_channels = channels_dict[res // 2] + 6
            elif res > 4:
    ...
    def forward(self, ws, c, **block_kwargs):
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f"b{res}")
            x, img = block(x, img, c, cur_ws, **block_kwargs)
        return img
