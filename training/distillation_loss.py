        ...
        if self.style_mixing_prob > 0:
            with torch.autograd.profiler.record_function("style_mixing"):
                ...
                ws[:, cutoff:] = self.G.mapping(
                    torch.randn_like(z), c, update_emas=False
                )[:, cutoff:]
    ...
    def accumulate_gradients(
        ...
        if phase in ["Gmain", "Gboth"]:
            with torch.autograd.profiler.record_function("Gmain_forward"):
                ...
                # Distillation loss -------
                gen_p_img = self.G_p(gen_z, gen_c)["image"]
                loss_Distill = self.distillation_loss(gen_img, gen_p_img)
                training_stats.report("Loss/Distill/loss", loss_Distill)
                loss_Gmain += loss_Distill * 10
                # -------
