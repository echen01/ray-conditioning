...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    if not training_set.has_labels:
        ...
        # Reorder.
        label_order = sorted(label_groups.keys(), reverse=True)
...
def training_loop(
    ...
    G_p = None
    if loss_kwargs["class_name"] == "training.distillation_loss.StyleGAN2Loss":
        "Loading parent model..."
        with dnnlib.util.open_url(loss_kwargs["parent_model"]) as f:
            G_p = (
                legacy.load_network_pkl(f)["G_ema"]
                .eval()
                .requires_grad_(False)
                .to(device)
            )
    ...
    for module in [G, D, G_ema, augment_pipe, G_p]:
    ...
    loss = dnnlib.util.construct_class_by_name(
        device=device, G=G, D=D, G_p=G_p, augment_pipe=augment_pipe, **loss_kwargs
    ...
    for name, module, opt_kwargs, reg_interval in [
        ...
        if reg_interval is None:
            if freeze and name == "G":
                params = module.synthesis.parameters()
            else:
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for real_img, real_c, gen_z, gen_c in zip(
                phase_real_img, phase_real_c, phase_gen_z, phase_gen_c
            ):
                # Warp images
                if homography_warp:
                    B = real_img.shape[0]
                    for i in range(B):
                        if True:  # random.random() < 0.2:
                            # Real images
                            c2w = real_c[i][:16].view(4, 4).cpu().numpy()
                            intrinsics = real_c[i][16:].view(3, 3).cpu().numpy()
                            cam = warp.Camera.Create(c2w, intrinsics)
                            warped_cam = warp.get_front_cam(cam)
                            real_c[i][:16] = torch.tensor(
                                warped_cam.mEx, device=real_c.device
                            ).view(-1)
                            warped_img = warp.transform_front(real_img[i], cam)
                            real_img[i] = warped_img

                            # Fake images
                            c2w = gen_c[i][:16].view(4, 4).cpu().numpy()
                            intrinsics = gen_c[i][16:].view(3, 3).cpu().numpy()
                            cam = warp.Camera.Create(c2w, intrinsics)
                            warped_cam = warp.get_front_cam(cam)
                            gen_c[i][:16] = torch.tensor(
                                warped_cam.mEx, device=gen_c.device
                            ).view(-1)
