...
import training.math_utils as math_utils
...
class LookAtPoseSampler:
    ...
    def sample(
        ...
        if truncation:
            h = torch.clamp(h, horizontal_mean - horizontal_stddev * 2, horizontal_mean + horizontal_stddev * 2)
            v = torch.clamp(v, vertical_mean - vertical_stddev * 2, vertical_mean + vertical_stddev * 2)
        v = torch.clamp(v, 1e-5, math.pi - 1e-5)
...
def create_cam2world_matrix(forward_vector, origin):
    ...
    up_vector = torch.tensor(
        [0, 1, 0], dtype=torch.float, device=origin.device
