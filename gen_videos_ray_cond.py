...
def gen_interp_video(
    ...
    **video_kwargs,
    ...
    # hard coded for afhq
    avg_camera_pivot = [0.0, 0.0, 0.2]
    avg_camera_radius = 2.7
    camera_lookat_point = torch.tensor(avg_camera_pivot, device=device)
    zs = torch.from_numpy(
        np.stack([np.random.RandomState(seed).randn(G.z_dim) for seed in all_seeds])
    ).to(device)
    cam2world_pose = LookAtPoseSampler.sample(
        3.14 / 2, 3.14 / 2, camera_lookat_point, radius=avg_camera_radius, device=device
    )
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                cam2world_pose = LookAtPoseSampler.sample(
                    3.14 / 2
                    + yaw_range
                    * np.sin(2 * 3.14 * frame_idx / (num_keyframes * w_frames)),
                    3.14 / 2
                    - 0.05
                    + pitch_range
                    * np.cos(2 * 3.14 * frame_idx / (num_keyframes * w_frames)),
                    camera_lookat_point,
                    radius=avg_camera_radius,
                    device=device,
                )
                all_poses.append(cam2world_pose.squeeze().cpu().numpy())
                focal_length = (
                    4.2647 if cfg != "Shapenet" else 1.7074
                )  # shapenet has higher FOV
                intrinsics = torch.tensor(
                ...
                if entangle == "conditioning":
                    ...
                    img = G.synthesis(ws=w_c, c=c_forward, noise_mode="const")[
                        image_mode
                    ][0]
                    ...
                    img = G.synthesis(w.unsqueeze(0), c=c[0:1], noise_mode="const")[0]
                elif entangle == "both":
                    w_c = G.mapping(
                        z=zs[0:1],
                        c=c[0:1],
                    ...
                    img = G.synthesis(ws=w_c, c=c[0:1], noise_mode="const")[image_mode][
                        0
                    ]
...
@click.option(
    "--shuffle-seed",
    type=int,
    help="Random seed to use for shuffling seed order",
    default=None,
)
@click.option(
    "--grid",
    type=parse_tuple,
    help="Grid width/height, e.g. '4x3' (default: 1x1)",
    default=(1, 1),
)
@click.option(
    "--num-keyframes",
...
@click.option(
    "--outdir", help="Output directory", type=str, required=True, metavar="DIR"
)
@click.option(
    "--reload_modules",
...
@click.option(
    "--cfg",
    help="Config",
    type=click.Choice(["FFHQ", "AFHQ", "Shapenet"]),
    required=False,
    metavar="STR",
    default="FFHQ",
    show_default=True,
...
@click.option(
    "--nrr",
    type=int,
    help="Neural rendering resolution override",
    default=None,
    show_default=True,
)
@click.option(
