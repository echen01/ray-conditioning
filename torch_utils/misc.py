    ...
    for name, tensor in named_params_and_buffers(dst_module):
        ...
        if name in src_tensors:
            if tensor.shape == src_tensors[name].shape:
                ...
                tensor[: tensor.shape[0] - encoding_size].copy_(
                    src_tensors[name].detach()
                ).requires_grad_(tensor.requires_grad)
            elif "conv" in name or "torgb" in name or "L" in name:
                tensor[:, : (tensor.shape[1] - encoding_size)].copy_(
                    src_tensors[name].detach()
                ).requires_grad_(tensor.requires_grad)
            else:
                print(f"different shape at {name}")
